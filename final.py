...
def first_shar(file):
    """
    file -> dict
    This function reads the file and returns dct which consist of four countries and number of films filmed
     in a chosen year
    """
    dct = dict()
    country_1 = []
...
def second_shar(file):
    """
    file -> lst
    This function reads file and returns the lst which consist of locations of chosen country and year
    """
    lst = []
    lst_main = []
    lst_end = []
...
def map_func(lst, dct):
    ...
    for i in lst:
        try:
            ...
            if location != None:
                ...
                point_1.add_child(folium.Marker(location=[
                                  lat, long], popup="Film was filmed here " + str(year_for_3), icon=folium.Icon()))
    ...
    for d in dct:
        try:
            ...
            if location != None:
                lat, long = location.latitude, location.longitude
                point_2.add_child(folium.Marker(location=[lat, long], popup="There were " + str(
                    dct[d]) + " films filmed in " + str(year),  icon=folium.Icon()))
            map.add_child(point_2)
        except GeocoderTimedOut:
            continue

    population_needed = folium.FeatureGroup(name="Population")
    population_needed.add_child(folium.GeoJson(data=open('world.json', 'r',
                                             encoding='utf-8-sig').read(),
                                   style_function=lambda x: {
                                       'fillColor': 'green' if x['properties']['POP2005'] < 10000000
                                       else 'orange' if 10000000 <= x['properties']['POP2005'] < 20000000
                                       else 'red'}))
    map.add_child(population_needed)
